...
describe('throwError middleware', () => {
  ...
  it('should throw WorkflowAIApiRequestError if response is 500', async () => {
    ...
    await expect(throwError.onResponse?.(response, {} as MergedOptions)).rejects.toThrow(WorkflowAIApiRequestError)
  })
  ...
  it('should not throw WorkflowAIApiRequestError if response status is 200', async () => {
    ...
    await expect(throwError.onResponse?.(response, {} as MergedOptions)).resolves.not.toThrow(WorkflowAIApiRequestError)
  })
  ...
  it('should throw WorkflowAIApiRequestError with correct error message', async () => {
    ...
    try {
      ...
      expect(error.message).toBe(`Failed to request ${response.url}: {"message":"Something went wrong"}`)
